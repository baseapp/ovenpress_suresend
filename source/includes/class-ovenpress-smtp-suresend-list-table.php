...
//namespace OVENPRESS\LIST\TABLE;

//use WP_List_Table;

class Ovenpress_SMTP_Suresend_List_Table extends WP_List_Table
{
    private $table_data;
    ...
    function get_columns()
    {
        $columns = array(
            ...
            'time'          => __('Time', 'OvenPress'),
            'recipient'         => __('Recipient', 'OvenPress'),
            'subject'   => __('Subject', 'OvenPress'),
            // 'attachments'   => __('attachments', 'OvenPress'),
            'headers'   => __('Headers', 'OvenPress'),
            'status'        => __('Status', 'OvenPress'),
            'action'        => __('', 'OvenPress')
        );
        return $columns;
    }
    ...
    {
        global $wpdb;
        $table_name = esc_sql($wpdb->prefix . 'ovenpress_smtp_suresend_logs');
        ...
        if (isset($_POST['resend'])) {
            ...
            if ($single_row) {
                $to          = $single_row->recipient;
                $subject     = $single_row->subject;
    ...
    function views()
    {
        ...
        echo sprintf(
            '<a type="button" style="color:blue;" href="?page=ovenpress-smtp-suresend-smtp-logs">All&nbsp;(%s)</a>&nbsp;|&nbsp;',
            $row_count
        );
        echo sprintf(
            '<a style="color:green;" href="?page=ovenpress-smtp-suresend-smtp-logs&action=success">Success&nbsp;(%s)</a>&nbsp;|&nbsp;',
            $success
        );
        echo sprintf(
            '<a style="color:red;" href="?page=ovenpress-smtp-suresend-smtp-logs&action=failed">Failed&nbsp;(%s)</a>&nbsp;',
            $failed
        );
        echo '</ul>';
    }
