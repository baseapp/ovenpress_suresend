(function ($) {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    var element = document.querySelector(
      "li.toplevel_page_ovenpress-smtp-suresend",
    );
    if (element) {
      element.remove();
    }
    ...
    async function suresendPurgeSMTPLogs() {
      const ajax_nonce = document.getElementById(
        "ovenpress-ajax-nonce",
      ).innerText;
      const ajax_url = document.getElementById("ovenpress-ajax-url").innerText;
      const response = await fetch(ajax_url, {
        method: "POST",
        headers: {
          "Content-Type": "application/x-www-form-urlencoded; charset=utf-8",
        },
        body: `action=ovenpress_run_smtp_purge_all_logs&security=${ajax_nonce}`,
    ...
    const dkimCopyButton = document.querySelector("input.dkim-copy");

    if (dkimCopyButton) {
      dkimCopyButton.addEventListener("click", function () {
        const textarea = document.getElementById("dns-content-textarea");
