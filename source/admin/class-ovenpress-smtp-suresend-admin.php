...
//use OVENPRESS\LIST\TABLE\OvenPress_SMTP_Suresend_List_Table;
 ...
 * @subpackage Ovenpress_Smtp_Suresend/admin
 */
 ...
 */
class Ovenpress_Smtp_Suresend_Admin
{
	 ...
	 */
	private $version;
	private $_ovenpress_smtp_suresend;
	...
	public function __construct($plugin_name, $version)
	{
		...
		if (!class_exists('Ovenpress_SMTP_Suresend_List_Table')) {
			require_once ABSPATH . 'wp-admin/includes/class-wp-list-table.php';
			require_once plugin_dir_path(dirname(__FILE__)) . '/includes/class-ovenpress-smtp-suresend-list-table.php';
		}
		...
		add_action('ovenpress_smtp_log_cleanup_event', [$this, 'ovenpress_cleanup_smtp_logs']);
		add_action('wp_ajax_ovenpress_run_smtp_purge_all_logs', [$this, 'ovenpress_purge_all_smtp_logs']);
	}
	...
	public function enqueue_styles()
	{
		 ...
		 * defined in Ovenpress_Smtp_Suresend_Loader as all of the hooks are defined
		 * in that particular class.
		 *
		 * The Ovenpress_Smtp_Suresend_Loader will then create the relationship
		 * between the defined hooks and the functions defined in this
		 * class.
		 */

		wp_enqueue_style($this->plugin_name, plugin_dir_url(__FILE__) . 'css/ovenpress-smtp-suresend-admin.css', array(), $this->version, 'all');
	}
	...
	public function enqueue_scripts()
	{
		...
		wp_enqueue_script($this->plugin_name, plugin_dir_url(__FILE__) . 'js/ovenpress-smtp-suresend-admin.js', array('jquery'), $this->version, false);
	}
...
?>
	...
	function createDkimPublicKeys($dkim_folder_path)
	{
		...
		$options = get_option(OVENPRESS_SMTP_SURESEND_SLUG);
	...
	function log_email($mailArray)
	{
		if ($mailArray['to']) {
			...
			if ($smtp_option == 'smtp') {
				// $mail = $this->ovenpress_smtp_check_connection();
				// if ($mail) {
				...
				$this->send_mail($to, $subject, $message, $headers, $attachments, $status, $text_format);
				$this->ovenpress_save_smtp_logs($to, $subject, $message, $headers, $status);
			}
		}
	}
	 ...
	 */
	function ovenpress_smtp_check_connection()
	{
		$options = get_option(OVENPRESS_SMTP_SURESEND_SLUG);
	 ...
	 * @since 1.0.2
	 */
	function ovenpress_purge_all_smtp_logs()
	{
	 ...
	 * @since 1.0.2
	 */
	function ovenpress_cleanup_smtp_logs()
	{
		global $wpdb;
		...
		$table = $wpdb->prefix . 'ovenpress_smtp_suresend_logs';
	 ...
	 */
	function ovenpress_save_smtp_logs($to, $subject, $message, $headers, $status)
	{
		global $wpdb;
		$options = get_option(OVENPRESS_SMTP_SURESEND_SLUG);
        ...
        background:#22c55e;color:#ffffff;font-size:34px;line-height:64px;font-weight:bold;">
								...
								<tr>
									<td>
										<h2 style="color:#111827;margin:0 0 10px;font-size:20px;">
											<?php esc_html_e('Congrats, test email was sent successfully!', 'OvenPress SMTP Suresend'); ?>
										</h2>
								...
								<tr>
									<td style="padding-top:30px;">
										<p style="margin:0;font-size:14px;color:#374151;">
											<strong>OvenPress SMTP Suresend</strong><br>
											a <a href="https://www.wpoven.com">OvenPress</a> product
										</p>
									</td>
								</tr>
							...
							<p style="font-size:11px;color:#9ca3af;margin-top:20px;">
								...
								printf(
									...
									esc_html__('Sent from %s', 'OvenPress SMTP Suresend'),
									esc_html($domain)
								);
								?>
							</p>
...
<?php
	 ...
	 */
	function ovenpress_smtp_suresend_general_settings($options)
	{
		$smtp_option = $options['smtp-method-option'] ?? null;
		...
		$smtp_logging_status = array(
			...
			'type'     => 'button_set',
			'title'    => esc_html__('SMTP Logs', 'OvenPress SMTP Suresend'),
			'desc'     => esc_html__('Enable or disable smtp mail logging.', 'OvenPress SMTP Suresend'),
			'options' => array(
				'0' => 'Disable',
				'1' => 'Enable'
			),
		...
		$log_retention = array(
			...
			'type'     => 'select',
			'title'    => esc_html__('Log Retention Period', 'OvenPress SMTP Suresend'),
			'desc'     => esc_html__('Choose how long SMTP logs should be kept.', 'OvenPress SMTP Suresend'),
			'options'  => array(
				'7'   => '7 Days',
				'30'  => '30 Days (default)',
				'90'  => '90 Days',
	 ...
	 */
	function ovenpress_smtp_suresend_smtp_test_settings($options)
	{
		$fields = array();
	...
	function validateDkimDomain($dkimDomain)
	{
		...
		if (!is_string($dkimDomain) || $dkimDomain === '') {
			return false;
		}
		...
		return preg_match($dkimDomainRegex, $dkimDomain) === 1;
	}

	/**
	 * Get and copy DKIM public key.
	 */
	function ovenpress_smtp_suresend_dkim_settings()
	{
		$options = get_option(OVENPRESS_SMTP_SURESEND_SLUG);
	...
	function setup_gui()
	{
		...
		$options = get_option(OVENPRESS_SMTP_SURESEND_SLUG);
		$opt_name = OVENPRESS_SMTP_SURESEND_SLUG;

		Redux::disable_demo();

		$args = array(
			'opt_name'                  => $opt_name,
			'display_name'              => 'OvenPress SMTP Suresend',
			'display_version'           => ' ',
			//'menu_type'                 => 'menu',
			'allow_sub_menu'            => true,
			//	'menu_title'                => esc_html__('OvenPress Plugins', 'OvenPress SMTP Suresend'),
			'page_title'                => esc_html__('OvenPress SMTP Suresend', 'OvenPress SMTP Suresend'),
			...
			'output'                    => false,
			'output_tag'                => false,
			//'footer_credit'             => 'Please rate OvenPress SMTP Suresend ★★★★★ on WordPress.org to support us. Thank you!',
			'footer_credit'             => ' ',
			'use_cdn'                   => false,
			'admin_theme'               => 'wp',
		...
		Redux::set_section(
			$opt_name,
			array(
				'title'      => esc_html__('SMTP Settings', 'OvenPress SMTP Suresend'),
				'id'         => 'smtp-suresend',
				'subsection' => false,
				'heading'    => ' SMTP Configuration Settings',
				'desc'       => 'OvenPress SMTP Suresend ensures reliable email delivery with easy SMTP setup, connection testing, debugging, and secure configuration for WordPress.',
				'fields'     => $this->ovenpress_smtp_suresend_general_settings($options),
			)
		);
		...
		Redux::set_section(
			$opt_name,
			array(
				'title'      => esc_html__('DKIM Settings', 'OvenPress SMTP Suresend'),
				'id'         => 'dkim-settings',
				'subsection' => true,
				...
				'icon'       => 'el el-cloud',
				'fields' => $this->ovenpress_smtp_suresend_dkim_settings(),
			)
		);
		...
		Redux::set_section(
			$opt_name,
			array(
				'title'      => esc_html__('SMTP Mail Test', 'OvenPress SMTP Suresend'),
				'id'         => 'smtp-test',
				'subsection' => true,
				'parent'     => 'smtp-suresend',
				...
				'icon'       => 'el el-envelope',
				'fields' => $this->ovenpress_smtp_suresend_smtp_test_settings($options),
			)
		);
		...
		Redux::set_section(
			$opt_name,
			array(
				'title'      => '<a href="admin.php?page=ovenpress-smtp-suresend-smtp-logs"  class="smtp-logs">
	...
	function smtp_logs()
	{
		echo '<div class="wrap"><strong> <h1 style="font-weight: 500; font-size: 24px;">OvenPress SMTP Logs</h1></strong>';
		echo '<form method="post">';

		$table = new Ovenpress_SMTP_Suresend_List_Table();
		$table->prepare_items();
	 ...
	 * Add a admin menu.
	 */
	function ovenpress_smtp_suresend_menu()
	{
		$to = '';
		$subject = '';
		$message = '';
		...
		add_submenu_page('ovenpress', 'SMTP Suresend', 'SMTP Suresend', 'manage_options', 'admin.php?page=ovenpress-smtp-suresend&tab=1');
		add_submenu_page('admin.php?page=ovenpress-smtp-suresend&tab=1', 'SMTP Logs', 'SMTP Logs', 'manage_options', OVENPRESS_SMTP_SURESEND_SLUG . '-smtp-logs', array($this, 'smtp_logs'));
	}

	/**
	 * Hook to add the admin menu.
	 */
	public function admin_main(Ovenpress_Smtp_Suresend $ovenpress_smtp_suresend)
	{
		$this->_ovenpress_smtp_suresend = $ovenpress_smtp_suresend;
		$this->createDkimPublicKeys($this->dkim_folder_path);
		add_action('admin_menu', array($this, 'ovenpress_smtp_suresend_menu'));
		$this->setup_gui();
	}
}
